...
const DirectTeam: React.FC = () => {
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-emerald-50/30 via-white to-emerald-50/20 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border-emerald-200/50 bg-white/70 backdrop-blur-xl shadow-lg ring-1 ring-amber-400/10">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-emerald-700/70 mb-1">Total Direct Members</p>
                <p className="text-3xl font-bold text-emerald-700">{stats.total}</p>
              </div>
              <UserPlus className="h-10 w-10 text-emerald-600/50" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-emerald-200/50 bg-white/70 backdrop-blur-xl shadow-lg ring-1 ring-amber-400/10">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-emerald-700/70 mb-1">Active Direct Members</p>
      ...
      <Card className="border-emerald-200/50 bg-white/70 backdrop-blur-xl shadow-lg ring-1 ring-amber-400/10">
        <CardHeader>
          <CardTitle className="text-xl text-emerald-800">Direct Team Members ({teamData.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {teamData.length === 0 ? (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {teamData.map((member) => (
                    <tr key={member._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Badge className={member.position === 'left' ? 'bg-purple-100 text-purple-800' : 'bg-pink-100 text-pink-800'}>
                          {member.position || 'N/A'}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
